...
\hypersetup{
  ...
  pdfauthor={Jean-Charles J. C. Tassan; Trent Slade; Mohamad Al-Zawahreh;
    Bertrand D. J.-F. Thébault; Manuel Martín Morales Plaza;
    Wilson John Sterking Lauret; Dominique Colin; Fatiha (Nisrine) Bouzid;
    Timothy Sullivan},
...
\author{
  \small
  \begin{tabular}{c}
  Jean-Charles J. C. Tassan\textsuperscript{1} \quad
  Trent Slade\textsuperscript{2} \quad
  Mohamad Al-Zawahreh\\
  Bertrand D. J.-F. Thébault \quad
  Manuel Martín Morales Plaza\\
  Wilson John Sterking Lauret \quad
  Dominique Colin\\
  Fatiha (Nisrine) Bouzid \quad
  Timothy Sullivan
  \end{tabular}\\[3pt]
  \small
...
The dialogical-time, mathematical-structure, CSNP/Wasserstein, and
endogenous-stability antecedents were developed across the wider team
\citep{tassan-thebault-treal,resrag-tcfq,csnp-wasserstein,
team-endogenous-stability}.
...
$0<\varepsilon_{\min}<\varepsilon_{\max}$, the stable set is
\begin{equation}
\Eband=
\left\{
(\mu^\RES,\mu^\RAG):
...
estimator, window, encoder, ground metric, and uncertainty. Its provenance
includes the dialogical-time work of Tassan and Thébault
\citep{tassan-thebault-treal}.
...
\label{hyp:capacity}
\end{hypothesis}
This is a warning condition, not proof of irreversible failure. Its
accumulation/propagation behavior and endogenous-governability lineage are
attributed to the wider team \citep{team-endogenous-stability}.
...
not as the canonical expansion of the protocol name. This terminology history
includes the work of Al-Zawahreh and the multi-author CSNP/Wasserstein
synthesis \citep{csnp-wasserstein}.
...
The JSON Canonicalization Scheme defined by RFC 8785 \citep{rfc8785}:
\begin{enumerate}
  \item sorts object keys lexicographically by UTF-16 code unit;
  \item preserves array order;
  \item serializes strings as well-formed UTF-8 JSON without insignificant
        whitespace;
  \item serializes finite numbers with the exact ECMAScript representation
        required by JCS; and
...
strict JSON parsing, structural, hash, and predecessor-link checks. Evidence
retrieval and full JSON Schema validation remain integration responsibilities.
...
to Trent Slade, developed in collaboration with Tassan. The CSNP,
dialogical-time, mathematical-structure, and endogenous-stability provenance is
attributed to the complete team: Mohamad Al-Zawahreh, Bertrand D. J.-F.
Thébault, Manuel Martín Morales Plaza, Wilson John Sterking Lauret, Dominique
Colin, Fatiha (Nisrine) Bouzid, and Timothy Sullivan, together with Tassan and
Slade \citep{csnp-wasserstein,tassan-thebault-treal,resrag-tcfq,
team-endogenous-stability}.

Tassan reviewed the consolidated work and confirmed on 27 July 2026 that all
members of the team should be included in its citation. The repository
therefore records all nine names as version 1.0.0 authors. The accompanying
provenance statement distinguishes theory origin, formalization and software,
and antecedent research lineages without implying that every author wrote
every line or implemented every artifact. CRediT describes contribution rather
than determining authorship \citep{brand2022credit}; more granular CRediT
roles, affiliations, and ORCID identifiers may be added when confirmed by the
...
\path{paper/CONTRIBUTIONS.md}
  & Confirmed authorship and provenance record.\\
